...
router.post('/registro', async (req, res) => {
  const { nombre, username, email, password, objetivo, nivel, peso_actual, altura, edad, sexo, actividad, observaciones, dieta_tipo, alimentos_no, lesiones } = req.body;
  ...
  try {
    const ur = dbRun('INSERT INTO users (username, password, role, nombre, email, estado) VALUES (?, ?, ?, ?, ?, ?)',
      [username.toLowerCase(), hash, 'cliente', nombre, email||'', 'pendiente']);
