...
router.get('/revision-semanal/:cliente_id', (req, res) => {
  try {
    ...
    // JOIN: series_log → sesiones_entreno (fecha) → ejercicios_dia (musculos)
    // Buscamos por nombre de ejercicio porque series_log no guarda el ejercicio_id.
...
//
// Body: { sugerencias: [{muscle, accion, delta, aplicable_auto}, ...] }
// (el frontend pasa las que vienen del endpoint /revision-semanal)
router.post('/aplicar-sugerencias/:cliente_id', coachOnly, (req, res) => {
  try {
    ...
    // Cargar rutina completa con ejercicios indexados por id
    ...
    for (const sug of sugerencias) {
      ...
      // Buscar ejercicios donde sug.muscle sea el TARGET (primer músculo).
      // Solo modificamos series del músculo principal del ejercicio para evitar
      // efectos colaterales en otros grupos.
      ...
      // Decidir signo y total a repartir
      let totalDelta;
      if (sug.accion === 'subir')  totalDelta = +Math.abs(sug.delta);
      else if (sug.accion === 'bajar')  totalDelta = -Math.abs(sug.delta);
      else if (sug.accion === 'deload') totalDelta = sug.delta; // ya viene negativo
      else continue;

      // Repartir delta entre ejercicios proporcionalmente a sus series actuales
      ...
      ejsDelGrupo.forEach((e, idx) => {
        ...
        if (idx === ejsDelGrupo.length - 1) {
          // Último ejercicio absorbe el resto para que la suma cuadre exactamente
          cuota = totalDelta - repartido;
        } else {
          cuota = Math.round(totalDelta * (e.series / totalSeriesActuales));
          repartido += cuota;
        }
        const nuevasSeries = Math.max(1, (e.series || 0) + cuota); // nunca bajar de 1
    ...
    // Notificar via SSE a coach y cliente para refrescar UI
...
// Helper interno: resolver grupo desde nombre suelto
